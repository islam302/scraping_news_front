  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-6">
        <motion.div className="mb-5 sm:mb-6" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
          <h2 className="text-lg sm:text-xl font-bold text-text-primary">{t('sitesTitle')}</h2>
          <p className="text-sm text-text-secondary mt-0.5">{sites.length} {sites.length !== 1 ? t('sitesConfigured') : t('siteConfigured')}</p>
        </motion.div>
            ...
            {sites.map((site, i) => (
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-text-muted truncate">{site.lang}</p>
                </div>
              </motion.a>
            ))}
          </div>
